...
# ---- New coordination-quality metric tests --------------------------------


def _hold(env, agent: str) -> dict:
    s = env._states[agent]
    return {"type": 4, "target": np.array([s.x, s.y], dtype=np.float32)}


def _interact(x: float, y: float) -> dict:
    return {"type": 2, "target": np.array([x, y], dtype=np.float32)}


def _move_to(x: float, y: float) -> dict:
    return {"type": 1, "target": np.array([x, y], dtype=np.float32)}


# 13. handoff_rate == 1.0 when Ghost reveals every target and Spot interacts
def test_handoff_rate_perfect() -> None:
    env = make_env(1)
    assert isinstance(env, HeteroQuadEnv)
    env.reset(seed=0)

    t0, t1 = env.tasks.targets
    for t, (tx, ty) in zip((t0, t1), ((10.0, 10.0), (15.0, 15.0))):
        t.x, t.y = tx, ty
        t.revealed = False
        t.revealed_by = None
        t.interacted = False

    # Ghost reveals t0 (Spot is far away at starting pos)
    env._states["ghost"].x, env._states["ghost"].y = 10.0, 10.0
    env.step({"spot": _hold(env, "spot"), "ghost": _hold(env, "ghost")})
    assert t0.revealed and t0.revealed_by == "ghost"

    # Spot interacts t0
    env._states["spot"].x, env._states["spot"].y = 10.0, 10.0
    env.step({"spot": _interact(10.0, 10.0), "ghost": _hold(env, "ghost")})
    assert t0.interacted

    # Ghost reveals t1
    env._states["ghost"].x, env._states["ghost"].y = 15.0, 15.0
    env.step({"spot": _hold(env, "spot"), "ghost": _hold(env, "ghost")})
    assert t1.revealed and t1.revealed_by == "ghost"

    # Spot interacts t1 — episode ends here
    env._states["spot"].x, env._states["spot"].y = 15.0, 15.0
    _, _, _, _, infos = env.step({"spot": _interact(15.0, 15.0), "ghost": _hold(env, "ghost")})
    assert t1.interacted
    assert infos["spot"]["role_metrics"]["handoff_rate"] == 1.0
    env.close()


# 14. handoff_rate == 0.0 when Spot reveals and interacts without Ghost
def test_handoff_rate_zero() -> None:
    env = make_env(1)
    assert isinstance(env, HeteroQuadEnv)
    env.reset(seed=0)

    t0, t1 = env.tasks.targets
    # t0 near Spot's start (1, 1); t1 and Ghost kept far away
    t0.x, t0.y = 1.5, 1.0
    t0.revealed = False
    t0.revealed_by = None
    t0.interacted = False
    t1.x, t1.y = 18.0, 18.0
    t1.revealed = False
    env._states["ghost"].x, env._states["ghost"].y = 18.0, 18.0

    # Spot's HOLD triggers update_reveals — Spot (first in dict) reveals t0
    env.step({"spot": _hold(env, "spot"), "ghost": _hold(env, "ghost")})
    assert t0.revealed and t0.revealed_by == "spot"

    # Spot interacts t0
    env._states["spot"].x, env._states["spot"].y = 1.5, 1.0
    _, _, _, _, infos = env.step({"spot": _interact(1.5, 1.0), "ghost": _hold(env, "ghost")})
    assert t0.interacted
    assert infos["spot"]["role_metrics"]["handoff_rate"] == 0.0
    env.close()


# 15. spot_interact_given_valid == 1.0 when Spot picks INTERACT every valid step
def test_spot_interact_given_valid_one() -> None:
    env = make_env(1)
    assert isinstance(env, HeteroQuadEnv)
    env.reset(seed=0)

    t0, t1 = env.tasks.targets
    # Both targets revealed and within interaction range of Spot at (1, 1)
    t0.x, t0.y = 1.5, 1.0
    t0.revealed = True
    t0.interacted = False
    t1.x, t1.y = 2.0, 1.0
    ...
    # Step 1: mask on (both targets available), Spot picks INTERACT → t0 done
    env.step({"spot": _interact(1.0, 1.0), "ghost": _hold(env, "ghost")})

    # Step 2: mask still on (t1 still revealed and near), Spot picks INTERACT → t1 done
    _, _, _, _, infos = env.step({"spot": _interact(1.0, 1.0), "ghost": _hold(env, "ghost")})

    assert infos["spot"]["role_metrics"]["spot_interact_given_valid"] == 1.0
    env.close()


# 16. ghost_scout_given_unrevealed > 0.5 when Ghost moves toward nearest unrevealed
def test_ghost_scout_given_unrevealed_cone() -> None:
    env = make_env(1)
    assert isinstance(env, HeteroQuadEnv)
    env.reset(seed=0)

    t0, t1 = env.tasks.targets
    # t0 to Ghost's left, t1 to Ghost's right — both unrevealed
    t0.x, t0.y = 2.0, 10.0   # left
    t0.revealed = False
    t1.x, t1.y = 18.0, 10.0  # right
    t1.revealed = False

    # Ghost at centre; Spot parked well away from both targets
    env._states["ghost"].x, env._states["ghost"].y = 10.0, 10.0
    env._states["spot"].x, env._states["spot"].y = 10.0, 1.0

    # Two steps: Ghost moves toward t0 (left). t0 stays unrevealed (starts 8 m away,
    # Ghost closes to 6.5 m then 5.0 m — both above the 3.6 m reveal radius).
    for _ in range(2):
        _, _, _, _, infos = env.step({
            "spot": _hold(env, "spot"),
            "ghost": _move_to(2.0, 10.0),
        })

    assert infos["spot"]["role_metrics"]["ghost_scout_given_unrevealed"] > 0.5
    env.close()
